...
//		ImageView cardImg = (ImageView) findViewById(R.id.bankcardImageView); 
        cardImg.setImageBitmap(BankPhoto.markedCardImage);
        LayoutParams para;  
        para = cardImg.getLayoutParams();  
        DisplayMetrics metrics = new DisplayMetrics();
        this.getWindowManager().getDefaultDisplay().getMetrics(metrics);
        para.height = (int)(metrics.widthPixels * 0.63804);
        ...
        if (!bRecoFailed) {
			if (capture.strNumbers != "") {
				...
				LinearLayout.LayoutParams resultLayoutParams = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT,
            ...
            LinearLayout.LayoutParams resultTVParams = new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT);
            resultTV.setText("银行卡号");
            resultTV.setTextSize(22.0f);
            resultTV.setTextColor(Color.BLACK);
            resultTV.setBackgroundColor(Color.TRANSPARENT);
            resultTV.setGravity(Gravity.CENTER_VERTICAL);
            cardNumLayout.addView(resultTV, resultTVParams);
            ViewUtil.setPadding(resultTV, "10dip", "5dip", "5dip", "5dip");
        	...
        	unrecFinalResult.setTextColor(Color.BLACK);
        	unrecFinalResult.setTextSize(22.0f);
			resultLayout.addView(unrecFinalResult);
			cardNumLayout.addView(resultLayout, resultParams);
        }
